<template>
  <table class="comparison-table w-full">
    <thead>
      ...
      <td :class="['comparison-table__cell', { 'comparison-table__cell--featured': featuredCol == index }]" v-for="(heading, index) in headings" :key="index">
        {{ heading }}
      </td>
    </thead>
    ...
    <tbody>
      <tr v-for="(row, index) in data" :key="index">
        <th class="comparison-table__cell px-7 text-left">{{ row.title }}</th>

        <td :class="['comparison-table__cell', { 'comparison-table__cell--featured': featuredCol == index }]" v-for="(col, index) in row.cols" :key="index">
          ...
          <svg class="w-8" xmlns="http://www.w3.org/2000/svg" viewBox="-4754.284 12505.018 30.082 30.082" v-else-if="col === false">
      ...
      <tr v-if="showEmptyRow">
        ...
        <td :class="['comparison-table__cell comparison-table__cell--empty', { 'comparison-table__cell--featured': featuredCol == index }]" v-for="(heading, index) in headings" :key="index"></td>
      </tr>
    </tbody>
  </table>
</template>
