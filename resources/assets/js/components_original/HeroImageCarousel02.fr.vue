<template>
  <div class="HeroImageStatic02 relative">
        ...
        class="staticHero">
        <div
          class="text-center p-4 pt-16 pb-40 bg-black-full-45 md:pt-20 lg:text-left lg:px-30 lg:py-40 lg:pb-64 xxxl:pt-52"
    ...
    <div class="footnotes text-center absolute w-full lg:flex">
      <!--
      <div v-if="options.footnoteOne" class="footnote-one mx-4 md:mx-auto md:w-2/3 lg:mx-0 lg:w-1/2">
        <a
          :href="options.footnoteOne.url"
          v-html="options.footnoteOne.text"
        ...
        <div class="mobile lg:hidden text-white">
          <div
            v-html="options.footnoteOne.text"
            class="text-xs mb-2 md:text-base md:mb-3">
          </div>
          <a
            :href="options.footnoteOne.url"
            class="secondary btn-home on-dark w-full">
            {{ options.footnoteOne.mobileText }}
          </a>
        </div>
      </div>
      -->
      <div v-if="options.footnoteTwo" class="footnote-two w-full hidden lg:block px-8 py-1 bg-grey-77 tracking-wider xxl:py-0">
        <a
          v-if="options.footnoteTwo.text === ''"
          :href="options.footnoteTwo.url"
          class="text-white text-1xl no-underline uppercase xxl:text-2xl">
        ...
        <a
          ...
          class="text-white text-2xl no-underline uppercase">
        </a>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goQuiz(e) {
        let url = e.target.href; // Get the URL for the form from theme settings.
        this.hideFreshChat();
