      ...
      <div class="relative">
        <transition @enter="enter" @leave="leave" :css="false">
          <div class="w-full" v-for="(title, index) in titles" :key="index" v-if="index == currentColumn">
            <div v-for="(heading, headingIndex) in superior.headings"
              :class="['heading flex items-center justify-between px-16 py-3 text-sm sm:text-base', { 'border-b': index + 1 == titles.length }, { 'featured bg-green-thrive text-white': headingIndex == 0 }]"
              :key="'heading-' + headingIndex">
              <div v-html="heading"></div>
              <div>
                <svg v-if="superior.data[index].cols[headingIndex] === true"
                  class="w-8 -my-2" xmlns="http://www.w3.org/2000/svg" viewBox="-4351.284 12696.018 30.082 30.082">
                ...
                <svg v-if="superior.data[index].cols[headingIndex] === false"
                  class="w-8 -my-2" xmlns="http://www.w3.org/2000/svg" viewBox="-4754.284 12505.018 30.082 30.082">
                ...
                <div v-else
                  class="data flex-none"
                  :class="[{ 'text-xs': headingIndex == 0 || superior.data[index].cols[headingIndex].indexOf('<span>') >= 0 }]"
                  v-html="superior.data[index].cols[headingIndex]">
                </div>
              </div>
            </div>
          </div>
...
<style lang="sass" scoped>
  .heading > div::v-deep > span
    @apply mr-2 font-bold block

    @media (min-width: 576px)
      @apply inline

  .data
    @apply text-right text-black-full

    ::v-deep span
      @apply block font-bold text-base

  .featured .data::v-deep span
    @apply text-white
</style>
